...
>>>>>>> 3d4edf20a6c2e84c9eb3b4b7176b82f122b3c56c
...
<<<<<<< HEAD
        ...
        Button(self.frame,text='Get database',command=(lambda: self.collect(hostEntry.get(),userEntry.get(),passEntry.get(),dbEntry.get(),tableEntry.get(),var.get()))).grid(row=7,column=0,padx=1,columnspan=2)
        self.enter=lambda event: self.collect(hostEntry.get(),userEntry.get(),passEntry.get(),dbEntry.get(),tableEntry.get(),var.get())
        tableEntry.bind('<Return>',self.enter)

        self.root.mainloop()
        
    def collect(self,host,user,password,db,table,device):
        try:
            self.root.destroy()
...
<<<<<<< HEAD
            cur=conn.cursor()
            sql="SELECT * FROM {0} WHERE id='{1}' OR id='{2}' AND time>'{2}'".format(table,str(invIdDict[device]),str(idDict[invIdDict[device]]),lastTimeTag)
